...
describe('Warmup - timers and asynchronous specs', function () {
	...
	*/
	it('0 - should understand why we need asynchronous specs (so that this spec doesnt just pass)', function (done) {
		setTimeout(function () {
			expect(1).toBe(1);
			done();
		}, 100);
...
describe('Closure', function (done) {
	...
	it('1 - should understand loop and closure', function (done) {
		...
		for (i = 0; i < 3; i += 1) {
			...
			//	}, 1000 * (i + 1));
			//}
			setTimeout(function(toAppend){
				//var label = '' + i;
				return function () {
					debugElement.append(toAppend);
				}
			}(i+1), 1000 * (i + 1));
		}
		var createHandler = function (toAppend) {
			return function(){
				debugElement.append(toAppend);
			}
		}
